...
def createMenu():
    ...
    menusize = tkinter.Menu(root, tearoff=0)
...
def prepareWindow():
    ...
    tkinter.Button(root, text="Restart", command=restartGame).grid(row=0, column=0, columnspan=cols, sticky=tkinter.N+tkinter.W+tkinter.S+tkinter.E)
    buttons = []
    for x in range(0, rows):
        buttons.append([])
        for y in range(0, cols):
            b = tkinter.Button(root, text=" ", width=2, command=lambda x=x,y=y: clickOn(x,y))
